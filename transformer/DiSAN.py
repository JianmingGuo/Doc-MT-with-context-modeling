...
# ---------------   DiSAN Interface  ----------------
def disan(rep_tensor, rep_mask, scope=None, keep_prob=1., is_train=None, wd=0.,
...
def directional_attention_with_dense(rep_tensor, rep_mask, direction=None, scope=None,
    ...
    with tf.variable_scope(scope or 'directional_attention_%s' % direction or 'diag'):
        ...
        with tf.variable_scope('attention'):  # bs,sl,sl,vec
            ...
            dependent = linear_f(rep_map_dp, ivec, False, scope='linear_dependent')  # bs,sl,vec
            dependent_etd = tf.expand_dims(dependent, 1)  # bs,1,sl,vec
            head = linear_f(rep_map_dp, ivec, False, scope='linear_head')  # bs,sl,vec
        ...
        with tf.variable_scope('output'):
            ...
            fusion_gate = tf.nn.sigmoid(
                linear_f(rep_map, ivec, True, 0., 'linear_fusion_i', False, wd, keep_prob, is_train) +
                linear_f(attn_result, ivec, True, 0., 'linear_fusion_a', False, wd, keep_prob, is_train) +
                o_bias)
...
def bn_dense_layer(input_tensor, hn, bias, bias_start=0.0, scope=None,
    ...
    with tf.variable_scope(scope or 'bn_dense_layer'):
        linear_map = linear_f(input_tensor, hn, bias, bias_start, 'linear_map',
                            False, wd, keep_prob, is_train)
        if enable_bn:
...
def linear_f(args, output_size, bias, bias_start=0.0, scope=None, squeeze=False, wd=0.0,
