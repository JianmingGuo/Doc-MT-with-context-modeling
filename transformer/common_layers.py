...
from transformer import expert_utils as eu
...
def layer_norm(x, filters=None, epsilon=1e-6, name=None, reuse=None):
    ...
    with tf.variable_scope(name, "layer_norm", [x], reuse=reuse):
...
def standardize_images(x):
    with tf.name_scope("standardize_images", [x]):
        ...
        x_variance = tf.reduce_mean(tf.square(x-x_mean), axis=[1, 2, 3], keep_dims=True)
...
def embedding(x, vocab_size, dense_size, name=None, reuse=None, multiplier=1.0):
    with tf.variable_scope(name, "embedding", [x], reuse=reuse):
