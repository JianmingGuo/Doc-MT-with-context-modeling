...
class Evaluator():
    ...
    def greedy_search(self, X):
        ...
        preds = np.ones([X.shape[0], 1], dtype=np.int32) * 2  # <S>
    ...
    def beam_search(self, X):
        ...
        beam_size, batch_size = config.test.beam_size, X.shape[0]  #4 64 
        inf = 1e10
        ...
        def get_bias_scores(scores, bias):
            ...
            """
            b = np.array([0.0] + [-inf] * (beam_size - 1)) #beam_size
            b = np.repeat(b[None, :], 13, axis=0)       ##[13, 4]
            b = np.repeat(b[None, :], batch_size * beam_size, axis=0)  # [batch * beam_size, 13, beam_size]
            # bias=false: score valid, b useless
            # bias=true: score useless, b valid
        ...
        # Get encoder outputs.
        encoder_output = self.sess.run(self.model.encoder_output, feed_dict={self.model.src_pl: X}) ##(832, 50, 512)
        context_output = self.sess.run(self.model.context_output, feed_dict={self.model.src_pl: X}) ## (64, 13, 512)

        # Prepare beam search inputs.
        encoder_output = np.repeat(encoder_output, beam_size, axis=0)  # shape: [batch_size * beam_size, 13, hidden_units]
        context_output = np.repeat(context_output, beam_size, axis=0)  # shape: [batch_size * beam_size, 13, hidden_units]
        # print(encoder_output.shape) ##(3328, 50, 512)
        # print(context_output.shape)  ##(256, 13, 512)

        preds = np.ones([batch_size * beam_size , 13, 1], dtype=np.int32) * 2  # [[<S>, <S>, ..., <S>]], shape: [batch_size * beam_size, 13, 1]
        # print(preds.shape)   #(256*13, 1)
        scores = np.array(([0.0] + [-inf] * (beam_size - 1)) * batch_size)  # [0, -inf, -inf ,..., 0, -inf, -inf, ...], shape: [batch_size * beam_size]
        scores = np.repeat(scores[None, :], 13, axis=0)
        scores = scores.transpose()   ##[256, 13]
        for i in range(config.test.max_target_length):
            ...
            last_k_preds, last_k_scores = \
                self.sess.run([self.model.k_preds, self.model.k_scores],
                              feed_dict={self.model.encoder_output: encoder_output,
                                         self.model.context_output: context_output,
                                         self.model.decoder_input: preds})  
                                         # [batch_size * beam_size, beam_size]    

            last_k_preds = get_bias_preds(last_k_preds, bias)     #(256, 13, 4)    ## preds[4]  bias[256,13]
            last_k_scores = get_bias_scores(last_k_scores, bias)  #(256, 13, 4)   ## score[4]  bias[256,13]
            print("#######")
            print(last_k_preds.shape)
            print(last_k_scores.shape)

            # Shrink the search range.
            scores = scores[:,:, None] + last_k_scores #[256,13,4]
            scores = scores.reshape([batch_size, 13, beam_size ** 2])  # [batch_size, 13, beam_size * beam_size]

            # Reserve beam_size nodes.
            k_indices = np.argsort(scores)[:,:, -beam_size:]  # [batch_size 64, 13, beam_size 4]
            k_indices = np.repeat(np.array(range(0, batch_size*13)), beam_size) * beam_size ** 2 + k_indices.flatten()  # [batch_size *13 * beam_size]
            scores = scores.flatten()[k_indices]  # [batch_size * 13 * beam_size]
            last_k_preds = last_k_preds.flatten()[k_indices]
            print("&&&&&&&&&&&")
            print(last_k_preds)
            preds = preds[k_indices // beam_size]
            preds = np.concatenate((preds, last_k_preds[:, :,None]), axis=1)  # [batch_size * beam_size,13, i]

        scores = scores.reshape([batch_size, 13, beam_size])
        preds = preds.reshape([batch_size, beam_size, -1])  # [batch_size, beam_size, 13, max_length]
    ...
    def translate(self):
        ...
        count = 0
        start = time.time()
        for X in self.du.get_test_batches_doc():
    ...
    def evaluate(self):
        ...
        # else:
        #     script_path = 'tools/multi-bleu.perl'
...
if __name__ == '__main__':
    ...
    # args = parser.parse_args([
    #     '--config', 'configs/config_generator_train_deen.yaml'
    # ])
    # Read config
