...
from transformer.common_attention import multihead_attention
from transformer.common_layers import layer_norm
...
def post_table(attn_vec, n_head, dropout, is_training, kernel_initializer):
    """Post-attention processing."""
    # post-attention projection (back to `d_model`)
    '''
    proj_o = tf.get_variable('t/kernel', [n_head, 1],
                             dtype=attn_vec.dtype, initializer=kernel_initializer,
                             trainable=True)
    '''
    #output = tf.einsum('ijbn,n->ijb', attn_vec, proj_o)
    shape = tf.shape(attn_vec)[:-1]
    #attn_vec = tf.reshape(attn_vec, [-1, n_head])
    output = tf.layers.dense(attn_vec, 1, "relu", kernel_initializer=kernel_initializer)
...
def abs_attn_core(q_head, k_head, v_head, attn_mask, dropatt, is_training,
    ...
    return attn_vec, attn_score #ijbn
...
def multihead_attn(q, k, v, attn_mask, d_model, n_head, d_head, dropout,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # attention vector
        attn_vec, attn_table = abs_attn_core(q_head, k_head, v_head, attn_mask, dropatt,
                                 is_training, scale) #ibnd, ijbn

        # post processing
        output = post_attention(v, attn_vec, d_model, n_head, d_head, dropout,
                                is_training, kernel_initializer, residual)
        attn_table = post_table(attn_table, n_head, dropout,
                                is_training, tf.initializers.random_uniform(minval=-0.5, maxval=0.5, seed=None))

    return output, attn_table
...
def summarize_sequence(summary_type, hidden, d_model, n_head, d_head, dropout,
                       dropatt, input_mask, is_training, initializer, summaries,
    ...
    with tf.variable_scope(scope, 'sequnece_summary', reuse=reuse):
        if summary_type == 'last':
            ...
            summary, attn_table = multihead_attn(summary_bias, hidden, hidden, input_mask,
                                     d_model, n_head, d_head, dropout, dropatt,
                                     is_training, initializer, residual=False)
            # [l, bsz, d], [1, l, bsz]
            '''
            summary = residual(hidden,
                               multihead_attention(
                                   query=summary_bias,
                                   memory=hidden,
                                   bias=input_mask,
                                   total_key_depth=d_head,
                                   total_value_depth=d_head,
                                   output_depth=d_head,
                                   num_heads=n_head,
                                   dropout_rate=dropatt if is_training else 0.0,
