...
def prepare_gan_dis_data(train_data_source, train_data_target, gan_dis_source_data, gan_dis_positive_data, num=None, reshuf=True):
...
def prepare_three_gan_dis_dev_data(gan_dis_positive_data, gan_dis_negative_data, gan_dis_source_data,
                                   dev_dis_positive_data, dev_dis_negative_data, dev_dis_source_data, num):
...
def print_string(src_or_trg, indexes, worddicts_r):
    sample_str = ''
...
def prepare_data(seqs_x, seqs_y, maxlen=None, n_words_src=30000, n_words=30000, precision='float32'):
    ...
    x_mask = np.zeros((maxlen_x, n_samples)).astype(precision)
    y_mask = np.zeros((maxlen_y, n_samples)).astype(precision)
...
def dis_three_length_prepare(seqs_x, seqs_y, seqs_xs, maxlen=50):
    ...
    y = np.zeros((2, n_samples)).astype('int32')
    xs = np.zeros((maxlen, n_samples)).astype('int32')
...
def deal_generated_samples(y_sample, dicts):
    ...
    n_samples = len(y_sample)  #n_samples = 192
    lens_y = [len(y) for y in y_sample] #[50,50,50,...] 192个值
    maxlen_y = np.max(lens_y)  #50
...
def deal_generated_samples_to_maxlen(y_sample, dicts, maxlen):
    ...
    y_mask = np.zeros((n_samples, maxlen)).astype('float32')
        ...
        if not reuse_var:
            with tf.device(init_device):
                ...
                matrix = tf.get_variable('W', [input_size, output_size], initializer=tf.constant_initializer(W_init),
                                         trainable=True)
                if use_bias:
                    bias_init = np.zeros((output_size,)).astype(precision)
